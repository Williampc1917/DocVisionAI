...
public class FirestoreService {
    ...
    /**
     * Saves a task under a user's document in the "Tasks" subcollection.
     *
     * @param task A Map containing task data like userId, title, and completed status.
     * @return The taskId of the newly created task.
     * @throws ExecutionException, InterruptedException
     */
    public String saveUserTask(Map<String, Object> task) throws ExecutionException, InterruptedException {
        String userId = (String) task.get("userId");
        String title = (String) task.get("title");

        // Create a new document under /Users/{userId}/Tasks/{auto-generated taskId}
        DocumentReference taskRef = firestore.collection("Users").document(userId).collection("Tasks").document();

        // Set additional fields like timestamp
    ...
    /**
     * Retrieves a list of tasks for a specific user by userId.
     *
     * @param userId The unique ID of the user whose tasks are being retrieved.
     * @return A list of tasks.
     * @throws ExecutionException If there is an error during the Firestore operation.
     * @throws InterruptedException If the Firestore operation is interrupted.
     */
    public List<Map<String, Object>> getUserTasks(String userId) throws ExecutionException, InterruptedException {
        List<Map<String, Object>> taskList = new ArrayList<>();

        // Access the Tasks subcollection under the user's document
        CollectionReference tasksRef = firestore.collection("Users").document(userId).collection("Tasks");

        // Get all task documents for the user
        ApiFuture<QuerySnapshot> future = tasksRef.get();
        QuerySnapshot taskDocuments = future.get();

        // Loop through each task document and add its data to the taskList
        for (DocumentSnapshot document : taskDocuments.getDocuments()) {
            Map<String, Object> taskData = document.getData();
            taskData.put("taskId", document.getId()); // Add task ID to the task data
    ...
    /**
     * Searches for a user in Firestore by userId.
     *
     * @param userId The unique identifier of the user to be searched.
     * @return A Map containing the user's data if found, or null if no user matches the userId.
     * @throws ExecutionException   If the Firestore operation fails.
     * @throws InterruptedException If the Firestore operation is interrupted.
     */
    public Map<String, Object> searchUserById(String userId) throws ExecutionException, InterruptedException {
     ...
     * @return The existing profile data as a Map, or null if the user is not found.
     * @throws ExecutionException, InterruptedException
     */
    public Map<String, Object> getUserProfile(String userId) throws ExecutionException, InterruptedException {
        ...
        if (document.exists()) {
            return document.getData(); // Return the user profile data
        } else {
     ...
     * @param profileData - A map of the profile data to save.
     * @throws ExecutionException, InterruptedException
     */
    public void saveUserProfile(String userId, Map<String, Object> profileData) throws ExecutionException, InterruptedException {
        DocumentReference docRef = firestore.collection("Users").document(userId);
        docRef.set(profileData); // Save the profile data
    }



    /**
     * Updates a task's completion status under a user's document in the "Tasks" subcollection.
     *
     * @param userId The ID of the user who owns the task.
     * @param taskId The ID of the task to be updated.
     * @param taskUpdate A Map containing the fields to update (e.g., completed status).
     * @throws ExecutionException If there is an error during the Firestore operation.
     * @throws InterruptedException If the Firestore operation is interrupted.
     */
    public void updateUserTask(String userId, String taskId, Map<String, Object> taskUpdate) throws ExecutionException, InterruptedException {
        ...
        // Update the task fields (in this case, completed field)
        taskRef.update(taskUpdate).get();
    }
