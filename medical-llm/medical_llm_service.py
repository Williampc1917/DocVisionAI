...
def start_new_session(patient_id):
    """
        Start a new conversation session for the specified patient.
    ...
    session_ref = db.collection('patients').document(patient_id).collection('conversations').document(session_id)
    session_ref.set({
...
def update_last_fetch_time(patient_id, session_id):
    """
...
def save_message_to_session(patient_id, session_id, role, content):
    """
    ...
    if current_size > FIRESTORE_LIMIT:
        ...
        session_id = start_new_session(patient_id)
        session_ref = db.collection('patients').document(patient_id).collection('conversations').document(session_id)
...
def get_conversation_history(patient_id, session_id):
    """
        Retrieve the conversation history for the given patient and session.
...
def get_current_session_id(patient_id):
    """
        Retrieve the latest session ID for the patient, or create a new session if none exists.
        ...
        Returns:
            str: The session ID.
        """
    sessions_ref = db.collection('patients').document(patient_id).collection('conversations')
    ...
    return latest_session.id if latest_session else start_new_session(patient_id)
...
def ask_ai_about_patient_reports(patient_id, question):
    """
        Handle the AI interaction by generating a prompt based on the patient's reports and the radiologist's question.
    ...
    """
    session_id = get_current_session_id(patient_id)
    conversation_history = get_conversation_history(patient_id, session_id)
    ...
    save_message_to_session(patient_id, session_id, "user", prompt)
    save_message_to_session(patient_id, session_id, "assistant", response)
